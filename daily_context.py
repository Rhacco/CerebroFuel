"""Daily long-horizon context cache for crypto-signal-monitor v3.2.6."""
...
def seasonality_from_dict(raw: Mapping[str, Any] | None) -> Seasonality:
    ...
    return Seasonality(
        ...
        current_score=(
            ...
            if item.get("current_score") is not None
            else None
        ),
...
def _stable_days(
    ...
    *,
    ...
    for day in raw_ranked:
        if day in selected:
            continue
        confidence = raw.weekday_confidence.get(day, 0.0)
        score = raw.weekday_scores.get(day, 0.0)
        strict_bootstrap = bootstrap and confidence >= 0.68 and score >= 0.075
        if enter.get(day, 0) >= enter_days or strict_bootstrap:
            selected.append(day)

    # If retained days exceed two, prefer the currently strongest evidence and then prior order.
    ...
    return tuple(selected), enter, exit_
...
def build_daily_coin_context(
    *,
    ...
    stable_days, enter, exit_ = _stable_days(
        raw,
        previous,
    ...
    stable = Seasonality(
        ...
        samples=raw.samples,
        source="daily-completed-days-hysteresis",
...
def carry_forward_context(
    *,
    ...
    if previous:
        ...
        # The weekday list remains stable; the current-day tendency is not guessed.
