"""Strict restore/checkpoint store for the independent v5.0.0 paper state."""
...
BRANCH = "paper-state-v500"
REMOTE_FILE = "paper_state_v500.json"
APP_VERSION = "5.0.0"
STATE_SCHEMA = 1
...
class GitHubStateStore:
    ...
    def request(self, method: str, path: str, payload: dict[str, Any] | None = None, *, allow_missing: bool = False) -> dict[str, Any] | None:
        ...
        request = Request(API + path, data=data, method=method, headers={
            ...
            "User-Agent": "cf-paper-state/5.0.0",
...
def _validate_state(raw: Any) -> dict[str, Any]:
    ...
    if raw.get("app_version") != APP_VERSION:
        raise RuntimeError("Paper-State gehört nicht zu v5.0.0")
...
def restore(path: Path, store: GitHubStateStore) -> int:
    if path.exists():
        try:
            ...
            print(f"Warnung: unbrauchbarer v5.0.0-Paper-State wurde verworfen ({exc}).", file=sys.stderr)
        else:
    ...
    if remote is None:
        print("Noch kein v5.0.0-Paper-Checkpoint vorhanden.")
        return 0
    try:
