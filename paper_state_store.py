# r1
"""Strict restore/checkpoint store for the independent v6.0.0 paper state."""
...
BRANCH = "paper-state-v600-r1"
REMOTE_FILE = "paper_state_v600_r1.json"
APP_VERSION = "6.0.0"
PACKAGE_REVISION = "r1"
STATE_SCHEMA = 4
...
class GitHubStateStore:
    ...
    def request(self, method: str, path: str, payload: dict[str, Any] | None = None, *, allow_missing: bool = False) -> dict[str, Any] | None:
        ...
        request = Request(API + path, data=data, method=method, headers={
            ...
            "User-Agent": "cf-paper-state/6.0.0",
...
def _validate_state(raw: Any) -> dict[str, Any]:
    ...
    if raw.get("app_version") != APP_VERSION or raw.get("package_revision") != PACKAGE_REVISION:
        raise RuntimeError("Paper-State gehört nicht zu v6.0.0 r1")
...
def restore(path: Path, store: GitHubStateStore) -> int:
    if path.exists():
        try:
            ...
            print(f"Warnung: unbrauchbarer v6.0.0-Paper-State wurde verworfen ({exc}).", file=sys.stderr)
        else:
    ...
    if remote is None:
        print("Noch kein v6.0.0-Paper-Checkpoint vorhanden.")
        return 0
    try:
