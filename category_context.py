"""Category rotation, laggard context and mixed signal selection for v3.4.1.
...
# v3.4.1: BTC header anchor, PAY without BTC double-counting, mixed top eight.
...
def select_category_entries(
    ...
    *,
    ...
    """Select the strongest entries and confirmed exits.

    Category quotas apply to entries.  Exits are capped separately and require
    the category-fading confirmation generated by the opportunity engine.  A
    strong red warning may therefore displace a weak blue entry, while final
    report formatting still places buys above sells.
    """
    ...
    for item in candidates:
        ...
        if bool(context.get("qualified_entry")):
            category = categories.get(code)
            if category is None or category.max_slots <= 0:
                continue
            if entry_counts.get(code, 0) >= category.max_slots:
                continue
            selected.append(item)
            entry_counts[code] = entry_counts.get(code, 0) + 1
        elif bool(context.get("qualified_exit")):
